...
test.describe("HeroCentered", () => {
  ...
  test("renders image frame regardless of whether image is provided", async ({ page }) => {
    ...
    await expect(page.locator("#centered-full .hero-centered__image-frame")).toBeVisible();
    await expect(page.locator("#centered-minimal .hero-centered__image-frame")).toBeVisible();
  });

  test("renders image when image prop is provided", async ({ page }) => {
    const section = page.locator("#centered-full");
    const img = section.locator(".hero-centered__image");
  ...
  test("renders placeholder when no image is provided", async ({ page }) => {
    ...
    await expect(section.locator(".hero-centered__image-placeholder")).toBeVisible();
    await expect(section.locator(".hero-centered__image")).toHaveCount(0);
  });
  ...
  test("renders badge when badgeLabel is provided", async ({ page }) => {
    const section = page.locator("#centered-full");
    const badge = section.locator(".hero-centered__badge-wrapper");
  ...
  test("hides badge when badgeLabel is absent", async ({ page }) => {
    const section = page.locator("#centered-minimal");
    await expect(section.locator(".hero-centered__badge-wrapper")).toHaveCount(0);
  });

  // ── Image position ─────────────────────────────────────────────────────────

  test("image frame appears after text block when imagePosition is below", async ({ page }) => {
    const inner = page.locator("#centered-full .hero-centered__inner");
    const children = inner.locator(":scope > *");
    // text-block is first, image-frame is second
    await expect(children.nth(0)).toHaveClass(/hero-centered__text-block/);
    await expect(children.nth(1)).toHaveClass(/hero-centered__image-frame/);
  });

  test("image frame appears before text block when imagePosition is above", async ({ page }) => {
    const inner = page.locator("#centered-above .hero-centered__inner");
    const children = inner.locator(":scope > *");
    // image-frame is first, text-block is second
    await expect(children.nth(0)).toHaveClass(/hero-centered__image-frame/);
    await expect(children.nth(1)).toHaveClass(/hero-centered__text-block/);
  });
  ...
  test("hides secondary CTA when secondaryCtaUrl is absent", async ({ page }) => {
    const section = page.locator("#centered-minimal");
    await expect(section.locator(".hero-centered__actions a")).toHaveCount(1);
  });

  test("hides primary CTA when ctaUrl is absent", async ({ page }) => {
    const section = page.locator("#centered-null");
    await expect(section.locator(".hero-centered__actions a")).toHaveCount(0);
  });
  ...
  test("renders without errors when all props are null", async ({ page }) => {
    const section = page.locator("#centered-null");
    await expect(section.locator(".hero-centered")).toBeVisible();
    await expect(section.locator("h1")).toHaveCount(0);
    await expect(section.locator(".hero-centered__copy")).toHaveCount(0);
    await expect(section.locator(".hero-centered__badge-wrapper")).toHaveCount(0);
    await expect(section.locator(".hero-centered__actions a")).toHaveCount(0);
    // Image frame still renders with placeholder
    await expect(section.locator(".hero-centered__image-frame")).toBeVisible();
    await expect(section.locator(".hero-centered__image-placeholder")).toBeVisible();
  });

  test("defaults to image-below layout when imagePosition is null", async ({ page }) => {
    const inner = page.locator("#centered-null .hero-centered__inner");
    const children = inner.locator(":scope > *");
    // No h1 or copy, but text-block is first, image-frame is second (below)
    await expect(children.nth(0)).toHaveClass(/hero-centered__text-block/);
    await expect(children.nth(1)).toHaveClass(/hero-centered__image-frame/);
  });
  ...
  test("text block is centered", async ({ page }) => {
    const textBlock = page.locator("#centered-full .hero-centered__text-block");
    // Centering is applied via the component's <style> block, not a Tailwind class
